...
-- import qualified  EFA.Action.Simulation as ActSim
...
main = do
  ...
  let flow = CubeSolve.solve topology etaAssignMap (etaMap tabEta)  demand 
      flow :: FlowTopo.Section Node (Result.Result (CubeMap.Data Base ND.Dim2 [] Double))
             
--  let powers = ActSim.getPowers grid flow         
