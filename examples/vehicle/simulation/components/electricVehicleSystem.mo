model electricVehicleSystem
  ...
  Modelica.Blocks.Math.Division division1 annotation(Placement(visible = true, transformation(origin = {-28.3333,31.6667}, extent = {{-5.59809,-5.59809},{5.59809,5.59809}}, rotation = 0)));
  ...
  Modelica.Blocks.Sources.Constant const_PowerDemand(k = powerDemand) annotation(Placement(visible = true, transformation(origin = {0.833278,73.3334}, extent = {{-6.1579,-6.1579},{6.1579,6.1579}}, rotation = 0)));
  Modelica.Blocks.Sources.Ramp ramp1(height = powerDemand, duration = 1) annotation(Placement(visible = true, transformation(origin = {-60.5556,74.7222}, extent = {{-12,-12},{12,12}}, rotation = 0)));
equation
  connect(ramp1.y,division1.u1) annotation(Line(points = {{-47.3556,74.7222},{-35.8333,74.7222},{-35.8333,35.0256},{-35.051,35.0256}}));
  connect(voltagesensor1.p,pin_n) annotation(Line(points = {{-79.7222,13.0088},{-80.2778,13.0088},{-80.2778,-20.2778},{-90,-20.2778}}));
  connect(voltagesensor1.v,division1.u2) annotation(Line(points = {{-73.5643,19.1667},{-73.5643,18.6111},{-35.8333,18.6111},{-35.8333,28.3078},{-35.051,28.3078}}));
  connect(pin_p,voltagesensor1.n) annotation(Line(points = {{-83.0556,42.7778},{-79.7222,42.7778},{-79.7222,31.1579},{-79.7222,25.3246}}));
  connect(division1.y,signalcurrent1.i) annotation(Line(points = {{-22.1754,31.6667},{-10,31.6667},{-10,35.5555},{-9.38243,35.5555}}));
