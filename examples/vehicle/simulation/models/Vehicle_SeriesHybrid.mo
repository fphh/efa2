model Vehicle_seriesHybrid
  ...
  Engine engine1(parMaxTorque = 170, parStartSpeed = 25, parDragTorque = 70, parEtaOpt = 0.35, parMaxSpeed = 500, engineFile = "maps/engine/base.txt") annotation(Placement(visible = true, transformation(origin = {-75.5206,0.291334}, extent = {{-13.2,-13.2},{13.2,13.2}}, rotation = 0)));
  ...
  ElectricGenerator generator(maxTorque = 200, maxPower = 15000, maxSpeed = 1000, maxEta = 0.95, inertia = 0.01, fileName = "maps/motor/base.txt") annotation(Placement(visible = true, transformation(origin = {-40,0.00000113911}, extent = {{8.19616,8.19616},{-8.19616,-8.19616}}, rotation = -180)));
  ...
  Battery battery1(resistance = 0.01, voltage = 200) annotation(Placement(visible = true, transformation(origin = {-76.3313,-52.2468}, extent = {{-10.9091,-10.9091},{10.9091,10.9091}}, rotation = 0)));
...
equation
  connect(electricVehicleSystem1.pin_p,battery1.pin_p) annotation(Line(points = {{-66.1885,-27.0903},{-81.3094,-27.0903},{-81.3094,-36.9449},{-81.3499,-36.9449},{-81.3499,-46.5917},{-81.9201,-46.5917}}));
  ...
  connect(battery1.pin_n,ground1.p) annotation(Line(points = {{-70.9448,-46.5935},{-68.4003,-46.5935},{-68.4003,-40.1448},{-61.6669,-40.1448}}));
  connect(battery1.controlbus1,motor.controlbus1) annotation(Line(points = {{-66.1003,-52.1305},{-44.7222,-52.1305},{-44.7222,-43.3333},{-31.9127,-43.3333},{-31.9127,-43.2201}}));
  connect(battery1.pin_p,motor.pin_p) annotation(Line(points = {{-81.9201,-46.5917},{-81.3499,-46.5917},{-81.3499,-36.9449},{-29.4137,-36.9449},{-29.4137,-38.2112}}));
  connect(battery1.pin_p,generator.pin_p) annotation(Line(points = {{-81.9201,-46.5917},{-81.3499,-46.5917},{-81.3499,-36.9449},{-34.4583,-36.9449},{-34.4583,-5.64637},{-34.6671,-5.64637}}));
  connect(integrator1.u,engine1.output_fuelPower) annotation(Line(points = {{-58.795,17.1369},{-61.7754,17.1369},{-61.7754,3.99857},{-64.2996,3.99857}}));
  ...
  connect(generator.pin_n,ground3.p) annotation(Line(points = {{-40.4912,-5.62767},{-40.8333,-5.62767},{-40.8333,-6.66948},{-52.3648,-6.66948}}));
  connect(generator.controlbus1,controller_serieshybrid1.controlbus1) annotation(Line(points = {{-32.2364,-0.614711},{4.16667,-0.614711},{4.16667,50.5556},{-8.61564,50.5556},{-8.61564,50.465}}));
  connect(generator.flange_shaft,engine1.flange_crankShaft) annotation(Line(points = {{-46.2515,-0.898298},{-64.1667,-0.898298},{-64.1667,-0.790217},{-64.4743,-0.790217}}));
  ...
  connect(controller_serieshybrid1.controlbus1,engine1.controlbus1) annotation(Line(points = {{-8.61564,50.465},{4.16667,50.465},{4.16667,29.1667},{-90.8333,29.1667},{-90.8333,1.11111},{-85,1.11111},{-85,1.38889},{-86.4077,1.38889},{-86.4077,2.16036}}));
