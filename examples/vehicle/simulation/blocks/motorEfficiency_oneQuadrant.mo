model MotorEfficiency
  ...
  Modelica.Blocks.Math.Abs abs_speed annotation(Placement(visible = true, transformation(origin = {-29.2511,16.1921}, extent = {{-7.45106,-7.45106},{7.45106,7.45106}}, rotation = 0)));
  Modelica.Blocks.Math.Division division_invertEfficiency annotation(Placement(visible = true, transformation(origin = {43.8503,36.5036}, extent = {{-5.59809,-5.59809},{5.59809,5.59809}}, rotation = 0)));
  Modelica.Blocks.Math.Gain gain_maxEta(k = maxEta) annotation(Placement(visible = true, transformation(origin = {13.7723,11.5762}, extent = {{-4.20593,-4.20593},{4.20593,4.20593}}, rotation = 0)));
  Modelica.Blocks.Nonlinear.Limiter limiter1(uMax = 1, uMin = 0.1) annotation(Placement(visible = true, transformation(origin = {24.7142,11.3468}, extent = {{-2.8727,-2.8727},{2.8727,2.8727}}, rotation = 0)));
equation
  connect(limiter1.y,switch_powerFlowDirection.u3) annotation(Line(points = {{27.8742,11.3468},{30.4653,11.3468},{30.4653,0.932612},{38.6012,0.932612},{38.6012,1.24846}}));
  connect(gain_maxEta.y,limiter1.u) annotation(Line(points = {{18.3988,11.5762},{21.6055,11.5762},{21.6055,11.3468},{21.267,11.3468}}));
  connect(limiter1.y,division_invertEfficiency.u2) annotation(Line(points = {{27.8742,11.3468},{30.3099,11.3468},{30.3099,33.1447},{37.1326,33.1447}}));
  connect(division_invertEfficiency.y,switch_powerFlowDirection.u1) annotation(Line(points = {{50.0082,36.5036},{54.7132,36.5036},{54.7132,18.0305},{33.8849,18.0305},{33.8849,9.32612},{38.6012,9.32612},{38.6012,9.39114}}));
  connect(combitable2d1.y,gain_maxEta.u) annotation(Line(points = {{6.46647,11.4869},{10.9244,11.4869},{10.9244,11.5762},{8.72521,11.5762}}));
  connect(const_one.y,division_invertEfficiency.u1) annotation(Line(points = {{7.70973,37.3576},{19.7479,37.3576},{19.7479,39.8624},{37.1326,39.8624}}));
  connect(abs_speed.y,combitable2d1.u1) annotation(Line(points = {{-21.055,16.1921},{-19.3277,16.1921},{-19.3277,15.9575},{-10.671,15.9575}}));
  connect(gain_speedScale.y,abs_speed.u) annotation(Line(points = {{-41.2305,47.3693},{-40.2077,47.3693},{-40.2077,16.1921},{-38.1924,16.1921}}));
