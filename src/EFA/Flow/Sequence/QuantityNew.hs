...
module EFA.Flow.Sequence.QuantityNew (
...
{-
graphFromSectionsNewSig ::
   (Node.C node, Unknown a) =>
   DataSequ.List (FlowTopo.Section node v) -> Graph node a v
graphFromSectionsNewSig sq =
   SeqFlow.Graph {
      storages =
         fmap
            (storageMapFromList (Fold.toList $ DataSequ.mapWithSection const sq) .
             StorageQuant.forwardEdgesFromSums) $
         Env.storageSequences $
         Fold.toList $ DataSequ.mapWithSection  (,) $
         fmap FlowTopo.topology sq,
      sequence = DataSequ.toMap sq
   }
-}
