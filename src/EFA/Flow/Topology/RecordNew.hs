...
module EFA.Flow.Topology.RecordNew (
   Flow(..),
   Section,
...
{-
flowTopologyFromRecordNew topo (SignalFlow.HRecord (Strict.Axis _ _ time) fs) =
   FlowTopoPlain.Section (SignalFlow.Data time) $
   Graph.fromMap (Graph.nodeLabels topo) $
   Map.unionsWith (error "flowTopologyFromRecord: duplicate edges") $
   Map.elems $
   Map.mapWithKey
      (\e@(Graph.DirEdge idx1 idx2) () ->
         let look = MapU.checkedLookup "Flow.flowTopologyFromRecord" fs
             normal   = look $ Topo.outPosFromDirEdge e
             opposite = look $ Topo.inPosFromDirEdge e
         in  case Arith.sign $ SignalFlow.sumData normal of
                Arith.Positive ->
                   Map.singleton
                      (Graph.EDirEdge $ DirEdge idx1 idx2)
                      (Just $ Flow {flowOut = normal, flowIn = opposite})
                Arith.Negative ->
                   Map.singleton
                      (Graph.EDirEdge $ DirEdge idx2 idx1)
                      (Just $ Flow {flowOut = SignalFlow.negateData opposite, 
                                    flowIn = SignalFlow.negateData normal})
                Arith.Zero ->
                   Map.singleton
                      (Graph.EUnDirEdge $ unDirEdge idx1 idx2)
                      Nothing) $
...
fromSectionNewSig ::
  (Sum a,
   ...
   FlowTopoPlain.Section node Graph.EitherEdge
      (SignalFlow.Data inst vec a) (FlowTopo.Sums (Result (SignalFlow.Data inst vec a)))
      (Maybe (Flow (SignalFlow.Data inst vec a))) ->
   FlowTopo.Section node (Result (SignalFlow.Data inst vec a))
fromSectionNewSig (FlowTopoPlain.Section dtime topo) =
   FlowTopoPlain.Section
      (Determined . SignalFlow.deltaData $ dtime)
      (Graph.mapEdge
         (fmap (fullFlow . fmap (Determined )))
         topo)
