...
class SipWith c1 c2 v1 v2 d1 d2 d3 where
  sipWith :: (d1 -> d2 -> d3) -> c1 (v1 d1) -> c2 (v2 d2)  -> c2 (v2 d3)

-- 0d - 0d
instance (VZipper d1 d2 d3 v1) => SipWith Scalar Scalar v1 v1 d1 d2 d3 where
         sipWith f (Scalar x) (Scalar y) = Scalar $ vzipWith f x y 

-- 0d - 1d
instance (VZipper d1 d2 d3 v1) => SipWith Scalar Signal v1 v1 d1 d2 d3 where
         sipWith f (Scalar (Value x)) (Signal y) = Signal $ vmap (f x) y 

-- 1d - 1d
instance (VZipper d1 d2 d3 v1) => SipWith Signal Signal v1 v1 d1 d2 d3 where
         sipWith f (Signal x) (Signal y) = Signal $ vzipWith f x y  

-- 1d - 2d
instance (VZipper d1 d2 d3 v1) => SipWith Signal Signal2 v1 v1 d1 d2 d3 where
         sipWith f (Signal x) (Signal2 y) = Signal2 $ vmap (vzipWith f x) y  

-- 2d - 2d
instance (VZipper d1 d2 d3 v1) => SipWith Signal2 Signal2 v1 v1 d1 d2 d3 where
...
instance (SipWith c1 c2 v1 v2 d1 d2 d3, DArith d1 d2 d3) => SArith (c1 (v1 d1)) (c2 (v2 d2)) (c2 (v2 d3)) where
  (.*) x y = sipWith (..*) x y
  (./) x y = sipWith (../) x y
  (.+) x y = sipWith (..+) x y
  (.-) x y = sipWith (..-) x y
