...
dtimes0 = M.fromList [(DTimeIdx sec rec0, S.fromList [1.0])]

sigs0 :: EnergyMap UTFSig
sigs0 = M.fromList [ (EnergyIdx sec rec0 0 1, S.fromList [3]) ]

dtimes1 :: DTimeMap UTFSig
dtimes1 = M.fromList [(DTimeIdx sec rec1, S.fromList [1.0])]


sigs1 :: EnergyMap UTFSig
sigs1 = M.fromList [ (EnergyIdx sec rec1 0 1, S.fromList [3.3]) ]
...
dtimes0s = M.fromList [ (DTimeIdx sec rec0, toScalar (Const 20.0)) ]

sigs0s :: EnergyMap InTermScalar
sigs0s = M.fromList [ (EnergyIdx sec rec0 0 1, toScalar (Const 3.0)) ]

dtimes1s :: DTimeMap InTermScalar
dtimes1s = M.fromList [ (DTimeIdx sec rec1, toScalar (Const 20.0)) ]

sigs1s :: EnergyMap InTermScalar
sigs1s = M.fromList [ (EnergyIdx sec rec1 0 1, toScalar (Const 3.3)) ]

-------------------------

dtimes0ss :: DTimeMap InTermScalar
dtimes0ss = M.fromList [ (DTimeIdx sec rec0, toScalar $ Atom $ DTime $ DTimeIdx sec rec0) ]

sigs0ss :: EnergyMap InTermScalar
sigs0ss = M.fromList [ (EnergyIdx sec rec0 1 0, toScalar $ Atom $ Energy $ EnergyIdx sec rec0 1 0) ]

dtimes1ss :: DTimeMap InTermScalar
dtimes1ss = M.fromList [ (DTimeIdx sec rec1, toScalar $ Atom $ DTime $ DTimeIdx sec rec1) ]

sigs1ss :: EnergyMap InTermScalar
sigs1ss = M.fromList [ (EnergyIdx sec rec1 0 1, toScalar $ Atom $ Energy $ EnergyIdx sec rec1 0 1) ]
