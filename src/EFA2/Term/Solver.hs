...
emptyLRPowerEnv idx = throwError (PowerIdxError idx)

emptyLRDPowerEnv :: LRDPowerEnv a
emptyLRDPowerEnv idx = throwError (DPowerIdxError idx)

emptyLREtaEnv :: LREtaEnv a
emptyLREtaEnv idx = throwError (EtaIdxError idx)

emptyLRDEtaEnv :: LRDEtaEnv a
emptyLRDEtaEnv idx = throwError (DEtaIdxError idx)

emptyLRXEnv :: LRXEnv a
emptyLRXEnv idx = throwError (XIdxError idx)
...
class Solver env where
      ...
      solve :: (Signal a, Arith a) => TheGraph [a] -> env [a] -> [(PowerIdx, [Val])] -> M.Map PowerIdx [a]
...
instance Solver AbsEnv where
         ...
         solve (TheGraph g _) (AbsEnv eenv xenv) given = M.union penv' res
           where gvs :: [EqTerm Abs]
                 ...
                 res = solveInTerms (mkEnv emptyLRDPowerEnv) (mkEnv emptyLRDEtaEnv) eenv xenv penv' inTs
...
solveInTerms :: (Arith a) 
                => DPowerEnv [a] -> DEtaEnv [a] -> EtaEnv [a] -> XEnv [a] -> M.Map PowerIdx [a] -> [InTerm] -> M.Map PowerIdx [a]
