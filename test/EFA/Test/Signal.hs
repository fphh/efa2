...
import EFA.Signal.Signal(TC(TC),UTSignal,UTSignal2,
                         interp1LinValid, interp2WingProfileValid)
...
check :: TC Signal.Sample (Typ UT UT UT) (Data Nil (Interp.Val Double)) ->
         ...
         Bool
check (TC (Data Invalid)) (TC (Data Invalid)) = True
check (TC (Data (Inter x1))) (TC (Data (Inter x2))) = valCheck x1 x2
check (TC (Data (Extra x1))) (TC (Data (Extra x2))) = valCheck x1 x2
check _ _ = False
